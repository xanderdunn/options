...
def parse_args(argv):
    ...
    parser.add_argument('--num_steps', help='Number of steps for which to run the experiment.', type=int, default=500000)
    ...
    parser.add_argument('--zeta', help='Intrinsic reward decay parameter.', type=float, default=0.5)
    parser.add_argument('--beta', help='RBF kernel width parameter.', type=float, default=80)
    parser.add_argument('--plan_interval', help='Execute value iteration after every n steps', type=int, default=10)
    parser.add_argument('--num_vi', help='Number of iterations of value iteration to perform.', type=int, default=2)
    parser.add_argument("--retain_theta", action='store_true', default=True, help="Retain previous value function estimate for VI.")
    parser.add_argument('--sim_samples', help='Number of sample start states from which to simulate options.', type=int, default=10)
    parser.add_argument('--sim_steps', help='Number of for which to simulate options.', type=int, default=20)
    ...
    parser.add_argument('--environment', help='Choose the environment.',
                        choices=['gridworld', 'gridworld_continuous', 'combo_lock'], default='gridworld')
    parser.add_argument('--gridworld_size', help='Gridworld is size * size', type=int, default=10)
    parser.add_argument('--failure_rate', help='The percent of actions in this environment that fail.', type=float, default=0.2)
