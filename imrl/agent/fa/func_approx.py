...
def tabular_function_approximator(state, num_states):
    """Given a state represented as an integer, return the \"one-hot vector\" that is all zeros and a 1 at the position representing the occupied state.  Because these discrete states are exactly represented, this is not truly an approximation.  It's assumed that the states start at 0.0."""
